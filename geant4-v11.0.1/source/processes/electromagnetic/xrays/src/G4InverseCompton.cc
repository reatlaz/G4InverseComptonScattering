...
G4InverseCompton::G4InverseCompton(G4LogicalVolume* anEnvelope,
  ...
  , fGammaTkinCut(0.0)
{
  ...
  fPlateNumber = n;
  if(verboseLevel > 0)
    G4cout << "### G4InverseCompton: the number of TR radiator plates = "
           << fPlateNumber << G4endl;
  if(fPlateNumber == 0)
  {
    G4Exception("G4InverseCompton::G4InverseCompton()", "VXTRELoss01",
  ...
  fAngleRadDistr = false; //! changed to false
  fCompton       = false;
...
///////////////////////////////////////////////////////////////////////////
G4InverseCompton::~G4InverseCompton()
{
...
void G4InverseCompton::ProcessDescription(std::ostream& out) const
{
...
//? обязательна
G4bool G4InverseCompton::IsApplicable(const G4ParticleDefinition& particle)
{
  return (particle.GetPDGCharge() != 0.0);
}

/////////////////////////////////////////////////////////////////////////////////
// Calculate step size for XTR process inside raaditor -- //? обязательная
G4double G4InverseCompton::GetMeanFreePath(const G4Track& aTrack, G4double,
...
//////////////////////////////////////////////////////////////////////////
// Interface for build table from physics list //? обязательная
void G4InverseCompton::BuildPhysicsTable(const G4ParticleDefinition& pd)
{
  if(pd.GetPDGCharge() == 0.)
  {
    G4Exception("G4InverseCompton::BuildPhysicsTable", "Notification",
...
void G4InverseCompton::BuildEnergyTable()
{
  ...
  for(iTkin = 0; iTkin < fTotBin; ++iTkin)  // Lorentz factor loop
  {
    G4PhysicsLinearVector* energyVector =
      new G4PhysicsLinearVector(fMinEnergyTR, fMaxEnergyTR, fBinTR);
...
void G4InverseCompton::BuildAngleForEnergyBank()
{
    ...
    for(iTR = 0; iTR < fBinTR; ++iTR)
    {
     ...
     // log-vector to increase number of thin bins for small angles
      ...
      for(i = fBinTR - 2; i >= 0; --i)
      {
        ...
        angleSum +=
          integral.Legendre10(this, &G4InverseCompton::SpectralAngleXTRdEdx,
...
void G4InverseCompton::BuildAngleTable()
{
  G4int iTkin, iTR;
  G4double energy;
...
G4PhysicsFreeVector* G4InverseCompton::GetAngleVector(G4double energy, G4int n)
{
...
// radiator
void G4InverseCompton::BuildGlobalAngleTable()
{
  G4int iTkin, iTR, iPlace;
  G4double radiatorCof = 1.0;  // for tuning of XTR yield
  G4double angleSum;
  ...
  for(iTkin = 0; iTkin < fTotBin; ++iTkin)  // Lorentz factor loop
  {
    ...
    for(iTR = fBinTR - 2; iTR >= 0; --iTR)
    {
      angleSum += radiatorCof * fCofTR *
                  integral.Legendre96(this, &G4InverseCompton::AngleXTRdEdx,
...
// passage through G4Envelope with discrete generation of G4Gamma //? обязаьтельная
G4VParticleChange* G4InverseCompton::PostStepDoIt(const G4Track& aTrack,
                                                  const G4Step& aStep)
{
  G4int iTkin;
  ...
  if(verboseLevel > 1)
  {
    G4cout << "Start of G4InverseCompton::PostStepDoIt " << G4endl;
  ...
  if(aTrack.GetVolume()->GetLogicalVolume() != fEnvelope)
  {
    if(verboseLevel > 0)
    {
      G4cout << "Go out from G4InverseCompton::PostStepDoIt: wrong volume "
             << G4endl;
    }
  ...
  else
  {
    ...
    if(iTkin == 0)  // Tkin is too small, neglect of TR photon generation
    {
      if(verboseLevel > 0)
      {
        G4cout << "Go out from G4InverseCompton::PostStepDoIt:iTkin = " << iTkin
               << G4endl;
      }
      return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
    }
...
G4complex G4InverseCompton::OneInterfaceXTRdEdx(G4double energy, G4double gamma,
                                                G4double varAngle)
{
...
G4double G4InverseCompton::SpectralAngleXTRdEdx(G4double varAngle)
{
...
G4double G4InverseCompton::AngleSpectralXTRdEdx(G4double energy)
{
...
G4double G4InverseCompton::AngleXTRdEdx(G4double varAngle)
{
  G4double result;
...
G4double G4InverseCompton::GetPlateFormationZone(G4double omega, G4double gamma,
                                                 G4double varAngle)
{
  G4double cof, lambda;
...
G4complex G4InverseCompton::GetPlateComplexFZ(G4double omega, G4double gamma,
                                              G4double varAngle)
{
...
// plate material
void G4InverseCompton::ComputePlatePhotoAbsCof()
{
...
G4double G4InverseCompton::GetPlateLinearPhotoAbs(G4double omega)
{
...
G4double G4InverseCompton::GetGasFormationZone(G4double omega, G4double gamma,
                                               G4double varAngle)
{
  G4double cof, lambda;
...
G4complex G4InverseCompton::GetGasComplexFZ(G4double omega, G4double gamma,
                                            G4double varAngle)
{
  G4double cof, length, delta, real_v, image_v;
...
// Computes matrix of Sandia photo absorption cross section coefficients for
// gas material
void G4InverseCompton::ComputeGasPhotoAbsCof()
{
  const G4MaterialTable* theMaterialTable = G4Material::GetMaterialTable();
...
// Returns the value of linear photo absorption coefficient (in reciprocal
// length) for gas
G4double G4InverseCompton::GetGasLinearPhotoAbs(G4double omega)
{
  G4double omega2, omega3, omega4;
...
// Omega is energy !!!
G4double G4InverseCompton::GetPlateZmuProduct(G4double omega, G4double gamma,
                                              G4double varAngle)
{
...
void G4InverseCompton::GetPlateZmuProduct()
{
...
// Omega is energy !!!
G4double G4InverseCompton::GetGasZmuProduct(G4double omega, G4double gamma,
                                            G4double varAngle)
{
...
// Calculates the product of linear cof by formation zone for gas.
// G4cout and output in file in some energy range.
void G4InverseCompton::GetGasZmuProduct()
{
...
G4double G4InverseCompton::GetPlateCompton(G4double omega)
{
...
G4double G4InverseCompton::GetGasCompton(G4double omega)
{
  G4int i, numberOfElements;
  G4double xSection = 0., nowZ, sumZ = 0.;
...
G4double G4InverseCompton::GetComptonPerAtom(G4double GammaEnergy, G4double Z)
{
...
// (matter1 -> matter2, or 2->1)
// varAngle =2* (1 - std::cos(theta)) or approximately = theta*theta
G4double G4InverseCompton::OneBoundaryXTRNdensity(G4double energy,
                                                  G4double gamma,
...
G4double G4InverseCompton::GetStackFactor(G4double energy, G4double gamma,
                                          G4double varAngle)
{
...
//////////////////////////////////////////////////////////////////////////////
// For photon energy distribution tables. Integrate first over angle
G4double G4InverseCompton::XTRNSpectralAngleDensity(G4double varAngle)
{
...
/////////////////////////////////////////////////////////////////////////
// For second integration over energy
G4double G4InverseCompton::XTRNSpectralDensity(G4double energy)
{
  fEnergy = energy;
  G4Integrator<G4InverseCompton, G4double (G4InverseCompton::*)(G4double)>
    integral;
  return integral.Legendre96(this, &G4InverseCompton::XTRNSpectralAngleDensity,
                             0.0, 0.2 * fMaxThetaTR) +
         integral.Legendre10(this, &G4InverseCompton::XTRNSpectralAngleDensity,
                             0.2 * fMaxThetaTR, fMaxThetaTR);
}

//////////////////////////////////////////////////////////////////////////
// for photon angle distribution tables
G4double G4InverseCompton::XTRNAngleSpectralDensity(G4double energy)
{
...
///////////////////////////////////////////////////////////////////////////
G4double G4InverseCompton::XTRNAngleDensity(G4double varAngle)
{
  fVarAngle = varAngle;
  G4Integrator<G4InverseCompton, G4double (G4InverseCompton::*)(G4double)>
    integral;
  return integral.Legendre96(this, &G4InverseCompton::XTRNAngleSpectralDensity,
...
// and angular tables
void G4InverseCompton::GetNumberOfPhotons()
{
  G4int iTkin;
...
// of a charged particle //? далее 4 обязательные
G4double G4InverseCompton::GetXTRrandomEnergy(G4double scaledTkin, G4int iTkin)
{
  ...
  {
    E1 = fProtonEnergyVector->GetLowEdgeEnergy(iTkin - 1); //для протона
    E2 = fProtonEnergyVector->GetLowEdgeEnergy(iTkin);
    W  = 1.0 / (E2 - E1); //мастабирование для электрона scaledTkin отмасштабированная энергия электрона в соответствие с энергией протона
    W1 = (E2 - scaledTkin) * W; //вес левого бина
    W2 = (scaledTkin - E1) * W; //вес правого бина
...
G4double G4InverseCompton::GetXTRenergy(G4int iPlace, G4double, G4int iTransfer)
{
...
G4double G4InverseCompton::GetRandomAngle(G4double energyXTR, G4int iTkin)
{
  G4int iTR, iAngle;
...
G4double G4InverseCompton::GetAngleXTR(G4int iPlace, G4double position,
                                       G4int iTransfer)
{
  G4double x1, x2, y1, y2, result;
