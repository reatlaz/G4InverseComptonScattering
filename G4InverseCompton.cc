...
void G4InverseCompton::BuildEnergyTable()
{
  ...
  for(iTkin = 0; iTkin < fTotBin; ++iTkin)  // Lorentz factor loop
  {
	...
	// integral.Legendre10(this, &G4InverseCompton::SpectralXTRdEdx,
      ...
      G4double hw = 1.23984197596 * keV;
      G4double avg = fGamma*fGamma*hw;
      G4double sigma = 0.05 * avg;
      G4double energyElement = energyVector->GetLowEdgeEnergy(iTR);
      G4double energy = std::fabs(std::exp(-0.5 * ((energyElement-avg)/sigma)*((energyElement-avg)/sigma))/(sigma * sqrt(2 * 3.14)));
      energySum += energy;
      G4cout << "energy =" << energy << ", energySum=" << energySum << G4endl;
